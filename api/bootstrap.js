...
export default async function handler(req,res){
 ...
 try{
  ...
  const normalized=leads.map(l=>{
    ...
    const contactLog=acts.filter(a=>["CONTACT","REACTIVATION"].includes(a.activity_type)).map(a=>{let d={};try{d=JSON.parse(a.body)}catch{}if(a.activity_type==="REACTIVATION")return{id:a.id,date:String(a.created_at).slice(0,10),type:"Reaktivācija",note:"Sales process sākts no jauna"+(d.lostReason?" · iepriekšējais LOST: "+d.lostReason:""),actorId:a.actor_id,at:a.created_at};return{id:a.id,date:d.date||String(a.created_at).slice(0,10),type:d.type||"Cits",note:d.note||a.body,actorId:a.actor_id,at:a.created_at}});
